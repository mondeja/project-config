...
class TreeFilesListenerMock(dict):
...
def test_Tree_generator(tmp_path):
    ...
    assert tree.files == {}  # files are cached when used
    assert isinstance(files, types.GeneratorType)

    assert next(files) == (path.as_posix(), expected_content)
    assert tree.files == {path.as_posix(): expected_content}
...
def test_Tree_file_caching(tmp_path):
    ...
    tree = Tree(tmp_path)
    tree.files = TreeFilesListenerMock()

    expected_files = {path.as_posix(): expected_content}
    files1 = tree.generator([path.name])
    assert tree.files == {}
    assert tree.files.setitem_calls == 0
    next(files1)
    assert tree.files.setitem_calls == 1
    assert tree.files == expected_files

    files2 = tree.generator([path.name])
    next(files2)
    assert tree.files.setitem_calls == 1  # no more __setitem__
    assert tree.files == expected_files
...
def test_Tree_directory(tmp_path):
    ...
    bar_fcontent == "bar"
    assert tree.files.setitem_calls == 2

    baz_fpath, baz_fcontent = next(fcontent)
    baz_fpath == baz_path.name
    baz_fcontent == "baz"
    assert tree.files.setitem_calls == 3
